...
import { generateAgentScope } from "@/lib/agent-scope"
...
async function refreshAgentScope(projectPath: string): Promise<void> {
  try {
    await generateAgentScope(projectPath)
    const projectId = await ensureProjectId(normalizePath(projectPath))
    await upsertWikiRegistryEntry(projectPath, projectId)
  } catch (err) {
    console.warn(
      "[ingest] agent-scope refresh failed:",

...
import {
  FileText, Users, Lightbulb, BookOpen, HelpCircle, GitMerge, BarChart3, TrendingUp, Target, ChevronRight, ChevronDown, Layout, Trash2,
} from "lucide-react"
        ...
        {sortedGroups.map(([type, items]) => {
          ...
          return (
            <div key={type} className="mb-1">
              ...
              {isExpanded && (
                <div className="ml-3 animate-in slide-in-from-left-1 fade-in duration-150">
                  {items.map((page) => {
                    ...
                    return (
                      ...
                      >
                        <div className="absolute left-0 top-1 bottom-1 w-[2px] rounded-full bg-accent/30 opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
                        <button
          ...
          {sources.map((file) => {
            ...
            return (
              <button
                ...
                className={`group relative flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-left text-sm ${
                  isSelected

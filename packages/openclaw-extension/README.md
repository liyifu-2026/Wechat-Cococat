...
- **A WeChat account logged in on your phone** â€? This account will be used by the bot. You must keep it logged in on your phone at all times. If you log out on the phone, the bot loses its connection.

- **A second screen** â€? WeChat login requires scanning a QR code with your phone camera. You'll need the QR code displayed on a screen separate from the phone running WeChat (e.g., a computer monitor, tablet, or another phone).

- **An agent-wechat server** â€? Either self-hosted via Docker or provided by someone else. You'll need the server URL and authentication token.

> **Note:** The agent-wechat container requires `SYS_PTRACE` and `seccomp=unconfined` (ptrace access to the WeChat desktop process). It cannot run in serverless or restricted container environments (AWS Fargate, Cloud Run, etc.) â€? use a VM or bare-metal Docker host.
...
Display the QR code on a screen separate from the phone running WeChat. WeChat's login QR scanner uses the camera only â€? it cannot scan from the phone's photo gallery.
...
You only need to do this once â€? the session persists across container restarts.
...
| `serverUrl` | string | â€? | agent-wechat REST API URL |
| `token` | string | â€? | Auth token (auto-read from `~/.config/agent-wechat/token` for local setups) |
...
```
OpenClaw Gateway
  â””â”€â”€ WeChat Monitor (polling loop)
        â”?
        â”?  GET /api/chats          (list chats with unreads)
        â”?  POST /api/chats/{id}/open  (open chat, clear unreads)
        â”?  GET /api/messages/{id}  (fetch new messages)
        â”?  GET /api/messages/{id}/media/{localId}  (download media)
        â”?  POST /api/messages/send (send reply)
        â”?
        â–?

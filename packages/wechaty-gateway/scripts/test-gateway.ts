 ...
 *   WECHATY_TOKEN=test \
 *   pnpm --filter @cococat/wechaty-gateway exec -- node --import tsx src/main.ts
 *
 * Step 2 — Run this client (in another terminal):
 *   pnpm --filter @cococat/wechaty-gateway exec -- node --import tsx scripts/test-gateway.ts
 */
...
const token = getArg('token') ?? process.env['WECHATY_TOKEN'] ?? readAuthToken()
if (!token) {
  console.error('No token found. Set WECHATY_TOKEN or create ~/.config/cococat/token (pnpm migrate from legacy)')
  process.exit(1)
}

...
import { WeChatClient, type WeChatClientOptions, ensureHostDataDirs, getAgentDataHostPath, getWeChatHomeHostPath, ensureAuthToken, readAuthToken, getAuthTokenPath, getCococatConfigDir } from "@cococat/shared";
...
/** Local image from `pnpm build:image` (see scripts/build-images-local.sh). */
function localImageForArch(): string {
  const m = os.machine?.() ?? process.arch;
  if (m === "arm64" || m === "aarch64") return "agent-wechat:arm64";
  return "agent-wechat:amd64";
}

function getImageTag(): string {
  if (process.env.AGENT_WECHAT_IMAGE) return process.env.AGENT_WECHAT_IMAGE;
  if (process.env.AGENT_WECHAT_USE_UPSTREAM === "1") {
    return `${GHCR_IMAGE}:${VERSION}`;
  }
  return localImageForArch();
}

function resolveImageOrExit(): string {
  const preferred = getImageTag();
  if (dockerImageExists(preferred)) return preferred;

  if (process.env.AGENT_WECHAT_USE_UPSTREAM === "1") {
    return pullUpstreamImage(preferred);
  }

  console.error(`Docker image not found: ${preferred}`);
  console.error("Build locally:  pnpm build:image   then   pnpm dev   (or pnpm cli up)");
  console.error("Upstream pull:  AGENT_WECHAT_USE_UPSTREAM=1 pnpm cli up");
  process.exit(1);
}

function pullUpstreamImage(preferred: string): string {
  let image = preferred;
  try {
    execSync(`docker image inspect ${image}`, { stdio: "ignore" });
    return image;
  } catch {
    console.log(`Image ${image} not found locally. Pulling from ghcr.io...`);
  }
  ...
  try {
    ...
    console.error(`Failed to pull ${image}. Check network and Docker setup.`);
    process.exit(1);
  }
}
...
// Auth token paths (canonical: ~/.config/cococat/token)
...
authCmd
  ...
  .action(async (opts: { regenerate?: boolean }) => {
    if (opts.regenerate) {
      fs.mkdirSync(getCococatConfigDir(), { recursive: true });
      const token = randomBytes(32).toString("hex");
      fs.writeFileSync(getAuthTokenPath(), token + "\n", { mode: 0o600 });
...
async function cmdUp(opts: { proxy?: string } = {}) {
  const image = resolveImageOrExit();
  ...
  const token = ensureToken();
  ensureHostDataDirs();
  const wechatHomeHost = getWeChatHomeHostPath();
  const agentDataHost = getAgentDataHostPath();
  ...
  const dockerArgs = [
    ...
    "-v", `${agentDataHost}:/data`,
    "-v", `${wechatHomeHost}:/home/wechat`,
    "-v", `${getAuthTokenPath()}:/data/auth-token:ro`,
  ];

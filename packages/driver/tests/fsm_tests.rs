...
#[tokio::test]
async fn test_mentioning_phase_defers_send_to_inputting() {
    let a11y = load_fixture("group_mention_popup.json");
    let identified = identify_states(&a11y, "");
    ...
    assert!(
        matches!(
            plan_state.phase,
            SendMessagePhase::Inputting | SendMessagePhase::Confirming
        ),
        "should advance past Mentioning, got {:?}",
        plan_state.phase
    );
    assert!(selected.is_some());
    let sel = selected.unwrap();
    ...
    match &sel.action {
        Action::Sequence { actions } => {
            let has_ctrl_a = actions
                .iter()
                .any(|a| matches!(a, Action::Key { combo } if combo == "ctrl+a"));
            assert!(
                !has_ctrl_a,
                "should not ctrl+a when @ mentions are already in the composer"
            );
            let type_action = actions.iter().find(|a| matches!(a, Action::Type { .. }));
            assert!(type_action.is_some(), "should type message body once");
            if let Some(Action::Type { text, .. }) = type_action {
                assert_eq!(text, &message);
            }
            let enter_count = actions
                .iter()
                .filter(|a| matches!(a, Action::Key { combo } if combo == "Return"))
                .count();
            assert_eq!(enter_count, 1, "should send exactly once");
        }
        other => panic!("expected Sequence, got {:?}", other),
    }
}

...
/// Proxy a noVNC WebSocket connection directly to the VNC port (bypassing websockify).
...
async fn handle_vnc_ws(ws: WebSocket, vnc_port: u16) {
    let vnc_addr = format!("127.0.0.1:{vnc_port}");

    let mut vnc = match TcpStream::connect(&vnc_addr).await {
        Ok(s) => s,
        Err(e) => {
            tracing::error!("Failed to connect to VNC at {vnc_addr}: {e}");
            return;
        }
    };

    let (mut ws_tx, mut ws_rx) = ws.split();
    let (mut vnc_rx, mut vnc_tx) = vnc.split();

    // Client WebSocket → VNC TCP
    let ws_to_vnc = async {
        while let Some(Ok(msg)) = ws_rx.next().await {
            match msg {
                Message::Binary(data) => {
                    if vnc_tx.write_all(&data).await.is_err() {
                        break;
                    }
                }
    ...
    // VNC TCP → Client WebSocket
    let vnc_to_ws = async {
        let mut buf = vec![0u8; 65536];
        loop {
            match vnc_rx.read(&mut buf).await {
                Ok(0) => break, // EOF
                Ok(n) => {
                    if ws_tx

...
fn find_dat_via_hardlink(
    ...
    let file_rows = query_wechat_db_params(
        &hardlink_db,
        hardlink_key,
    ...
    let dir_rows = query_wechat_db_params(
        &hardlink_db,
        hardlink_key,
        "SELECT rowid, username FROM dir2id WHERE rowid IN (?1, ?2);",
        &[&dir1, &dir2],
    );
...
fn get_emoji_media(
    ...
    if let Some(emoticon_key) = keys.get("emoticon.db") {
        ...
        let rows = query_wechat_db_params(
            &emoticon_db,
            emoticon_key,
            "SELECT cdn_url FROM kNonStoreEmoticonTable WHERE md5 = ?1 LIMIT 1;",
            &[&md5_val],
        );
...
fn get_voice_data(
    ...
    for (db_name, media_key) in &media_dbs {
        ...
        let name_rows = query_wechat_db_params(
            &media_db,
            media_key,
        ...
        let voice_rows = query_wechat_db_params(
            &media_db,
            media_key,

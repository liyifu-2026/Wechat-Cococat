...
pub fn spawn_event_monitor() {
    tokio::spawn(async move {
        ...
        let mut last_extract_attempt: Option<Instant> = None;
        const MIN_EXTRACT_INTERVAL_SECS: u64 = 10;
        ...
        loop {
            ...
            if needs_extract {
                let on_cooldown = last_extract_attempt
                    .map(|t| t.elapsed().as_secs() < MIN_EXTRACT_INTERVAL_SECS)
                    .unwrap_or(false);
                if !on_cooldown {
                    last_extract_attempt = Some(Instant::now());
                    if let Some(pid) = find_wechat_pid() {
                        let extracted = extract_keys_async(pid).await;
                        if !extracted.is_empty() {
                            let db = get_db();
                            store_keys(&db, &session.id, &account_dir, &extracted);
                            tracing::info!("[events] key extraction succeeded for {account_dir}");
                        }
                    }
                }
                let db = get_db();

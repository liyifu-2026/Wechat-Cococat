# Claude Context for CocoCat

> **注意**：历史 fork 名 `agent-wechat` / `pi-wechat` 已废弃。术语与包布局以仓库根 [`CONTEXT.md`](./CONTEXT.md) 为准；Agent 运维见 [`packages/agent/README.md`](./packages/agent/README.md)；队列与八项修复见 [`docs/PLAN-agent-queue.md`](./docs/PLAN-agent-queue.md)。

## What This Project Is

**CocoCat** — 微信 AI 伙伴。WeChat 在 Docker 容器内由 **CocoCat Driver**（Rust FSM）操控；**CocoCat Agent**（宿主机 Node/pi-agent）负责 LLM 与 per-chat 状态。可选 **Redis + BullMQ** 入站队列与定时主动消息。
...
**Layer map:** [`CONTEXT.md`](./CONTEXT.md) — container **Driver** + host **Agent** + optional Redis queue.

## Packages

```
packages/driver/     # CocoCat Driver (FSM + REST + WS) — no LLM
packages/agent/      # CocoCat Agent (pi-agent-core) — host brain
packages/shared/     # WeChatClient + types
packages/cli/        # wx CLI on host
apps/console/        # CocoCat Console (Tauri)
```

FSM paths below are relative to **`packages/driver/`** (Rust crate).

## CocoCat Agent (host)

| Module | Path | Role |
|--------|------|------|
| **monitor** | `packages/agent/src/monitor.ts` | WS events；`REDIS_URL` 时入队 |
| **queue** | `packages/agent/src/queue/` | BullMQ inbound/outbound workers |
| **session** | `packages/agent/src/session.ts` | Per-chat pi `Agent` |
| **tools** | `packages/agent/src/tools.ts` | `wechat_send_message`, `wechat_send_image`, schedule_* |

Run: `pnpm agent` (after `pnpm stack start driver`). 设 `REDIS_URL` 启用队列。

容器内 **不跑 LLM**。物理发送经 Driver `POST /api/messages/send`。
...
All paths above are relative to **`agent-server-rust/`** (fork authoritative tree).

...
```
Client (CLI / pi-wechat / Wechaty / custom)
    │
    │  REST API + WS (port 6174)
    ↓
agent-server (Rust) — channel driver only, no LLM
    │
    ├── Read: SQLite DBs → messages, contacts, chat list
    ├── Write: FSM plans → xdotool/xclip → WeChat UI
    ├── Events: /api/ws/events → new_messages notifications
    └── Media: WeChat file system → decode → serve via API
```

## pi Agent (host)

Auto-reply and reasoning run in **`packages/agent`** on the host using [pi-agent-core](https://github.com/earendil-works/pi). It subscribes to `/api/ws/events`, reads messages via REST, and sends replies via `POST /api/messages/send`.

- **FSM Plan** (`plans/`) — deterministic UI automation in the container
- **pi Agent** (`packages/agent/`) — LLM + tools on the host

See [Pi Agent Setup](/integrations/pi/setup) and [Architecture Layers](/how-it-works/architecture-layers/).

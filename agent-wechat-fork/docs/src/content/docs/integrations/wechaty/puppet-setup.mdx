...
Gateway mode wraps the puppet in gRPC, so existing Wechaty bots can connect using the standard `wechaty-puppet-service` — no `@agent-wechat/wechaty-puppet` dependency needed. This is the recommended approach for [Managed Hosting](/hosting/managed-hosting) instances, where the gateway is already running.

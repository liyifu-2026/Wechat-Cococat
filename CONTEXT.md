...
**CocoCat config root**  
用户级配置目录：Unix/macOS `~/.config/cococat/`，Windows `%APPDATA%\CocoCat\`（token、env、persona、bridge、wiki-registry 等）。
...
**CocoCat runtime resource root**
Console 启动栈时读取的只读运行资源根。开发态为仓库根；安装态为 Tauri bundle 的 `runtime/` 资源目录，包含 `docker-compose.yml`、`scripts/`、`config/`、`packages/agent/dist` 与 Agent 生产 `node_modules`。
_Avoid_: 新功能直接拼 `CARGO_MANIFEST_DIR/../../..` 或假设用户保留源码目录
...
**CocoCat data root**  
用户级数据目录：Unix/macOS `~/.local/share/cococat/`，Windows `%LOCALAPPDATA%\CocoCat\`（`chats/`、`memory/` 等）。
